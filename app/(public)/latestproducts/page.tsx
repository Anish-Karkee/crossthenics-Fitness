...
import { ShoppingBag, SlidersHorizontal, RotateCcw, Flame, Sparkles } from "lucide-react";
...
const LatestProducts = () => {
  ...
  const handleAddToCart = (productName: string) => {
    toast.success(`${productName} added to cart!`, {
      description: "Latest release reserved in your gear bag.",
      style: {
        background: "#0a0a0e",
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              <h1 className="text-4xl sm:text-6xl md:text-7xl font-black uppercase tracking-tighter text-[#F5F3EF] leading-none">
                LATEST <span className="text-electric-red">PRODUCT DROPS</span>
              </h1>
              <p className="mt-3 text-sm sm:text-base text-[#A8A6A1] max-w-2xl font-normal">
                The newest generation of hybrid velcro belts, heavy-duty carabiners, and dipped chains.
              </p>
            </div>
          ...
          {filteredProducts.length === 0 ? (
            <div className="col-span-full rounded-2xl border border-white/10 bg-white/[0.02] p-12 text-center">
              <p className="text-base text-[#A8A6A1]">
                No releases match your search or filter range.
              </p>
              <button
                onClick={() => {
                  setMinPrice("");
