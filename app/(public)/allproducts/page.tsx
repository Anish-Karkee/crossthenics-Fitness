...
import { ShoppingBag, SlidersHorizontal, RotateCcw, Sparkles } from "lucide-react";
...
const AllProducts = () => {
  ...
  const handleAddToCart = (productName: string) => {
    toast.success(`${productName} added to cart!`, {
      description: "Item reserved in your gear bag.",
      style: {
        background: "#0a0a0e",
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              <h1 className="text-4xl sm:text-6xl md:text-7xl font-black uppercase tracking-tighter text-[#F5F3EF] leading-none">
                ALL <span className="text-electric-red">GEAR & EQUIPMENT</span>
              </h1>
              <p className="mt-3 text-sm sm:text-base text-[#A8A6A1] max-w-2xl font-normal">
                Competition belts, tactical 900D backpacks, pure magnesium chalk, and calisthenics hardware.
              </p>
            </div>
            <div className="shrink-0">
              <span className="inline-flex items-center rounded-xl glass-pill px-4 py-2 text-xs font-bold uppercase tracking-wider text-[#F5F3EF]/80">
                {filteredProduct.length} {filteredProduct.length === 1 ? "Product" : "Products"} Available
              </span>
            </div>
          </div>
        </div>
          ...
          {filteredProduct.length === 0 ? (
            <div className="col-span-full rounded-2xl border border-white/10 bg-white/[0.02] p-12 text-center">
              <p className="text-base text-[#A8A6A1]">
                No products match your search or filter range.
              </p>
              <button
                onClick={() => {
                  setMinPrice("");
              ...
              >
                ...
                <div>
                  <div className="relative aspect-square w-full overflow-hidden rounded-2xl bg-[#0B0C0E] border border-white/5 p-4 flex items-center justify-center">
                    ...
                    />
                    <div className="absolute top-2.5 left-2.5 z-10 rounded-md border border-white/10 bg-black/80 px-2 py-0.5 text-[9px] font-black uppercase tracking-widest text-[#FF4500] backdrop-blur-md">
                      Pro Grade
                    </div>
                  </div>
